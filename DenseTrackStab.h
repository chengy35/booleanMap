...
public:
    ...
    std::vector<float> saliency; //saliency of each point 
    std::vector<float> averageSaliency; //saliency of average saliency
    ...
    Track(const Point2f& point_, const TrackInfo& trackInfo, const DescInfo& hogInfo,
        ...
        : point(trackInfo.length+1), disp(trackInfo.length), hog(hogInfo.dim*trackInfo.length),
          hof(hofInfo.dim*trackInfo.length), mbhX(mbhInfo.dim*trackInfo.length), 
          mbhY(mbhInfo.dim*trackInfo.length), saliency(trackInfo.length+1), averageSaliency(trackInfo.length+1)
    {
        index = 0;
        point[0] = point_;
    }
