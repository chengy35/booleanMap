...
//--------------------------------begin--------------------------------
//-------------------------modified by ChengYang-----------------------
/* tracking interest points by median filtering in the optical field */
void OpticalFlowTracker(const Mat& flow, // the optical field
...
{
	IplImage flows = IplImage ( flow );//不进行数据copy；
	CvMat salmat = salMap;/*Mat -> CvMat, 类似转换到IplImage，不复制数据只创建矩阵头*/
	CvMat *s = &salmat;
	...
	int width = flows.width;
	int height = flows.height;
	...
	for(int i = 0; i < points_in.size(); i++) {
		...
		for(int n = y-1; n <= y+1; n++) {
			...
			const float* f = (const float*)(flows.imageData + flows.widthStep*q);
			xs.push_back(f[2*p]);
			ys.push_back(f[2*p+1]);
			f = (const float*)(s->data.ptr + q * s->step);
			ss.push_back(f[p]);
		}
